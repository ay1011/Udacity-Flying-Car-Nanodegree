...
class BackyardFlyer(Drone):
    ...
    def __init__(self, connection):
        ...
        self.all_waypoints = self.calculate_box() #self.calculate_circle(7)
    ...
    def local_position_callback(self):
        ...
        if self.flight_state == States.TAKEOFF:
            ...
            # coordinate conversion
            local_position[2] = -1.0 * local_position[2]

            err_pos = np.linalg.norm(local_position - self.target_position)
            # if position error is below desired error, move to the next waypoint
            if abs(err_pos) < self.desired_error:
                self.waypoint_index += 1
                # check for terminal waypoint and land
    ...
    def waypoint_transition(self):
        ...
        print("target_position ",self.waypoint_index,": ", self.target_position)
